...
function imprimeResultadoNoInput(resultados) {
    ...
    labelResultado.innerHTML = (resultados.length) + " Resultados";

	var resultadosComoArray = resultados.map(function(item){ 
		return item.resultado;
	});
	
	labelResultado.innerHTML = (resultadosComoArray.length) + " Resultados";
    ...
    if(resultadosComoArray.length > 0) {
    	...
    	inputResultado.placeholder = 'Sem Resultados';
...
function limparResultados() {
	console.clear();
	document.querySelector('#labelResultados').innerHTML = '0 Resultados';
